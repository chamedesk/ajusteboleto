...
app = Flask(__name__)
app.secret_key = "ajuste-foto-boleto-web"
...
def ajustar_pdf(input_path, left=5, top=5, right=15, bottom=5, only_meter_images=True):
    output_name = f"{os.path.splitext(os.path.basename(input_path))[0]}_AJUSTADO.pdf"
            ...
            for rect in rects:
                ...
                if only_meter_images:
                    if rect.x0 > page_width * 0.45:
                        continue
                    if rect.y0 < page_height * 0.12 or rect.y1 > page_height * 0.72:
                        continue
...
def index():
    if request.method == "POST":
        ...
        input_path = os.path.join(UPLOAD_DIR, f"{file_id}_{pdf.filename}")
        pdf.save(input_path)
